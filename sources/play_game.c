...
/*   Updated: 2017/06/06 17:02:49 by jcharloi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_size(t_game *game, int y, int x)
{
...
void	play(t_game *game, int close, int y)
{
	int x;
	...
	while (y < game->y_board)
	{
		...
		{
			if (check_size(game, y, x) == 0 && who_is_my_ennemy(game, y, x))
			{
				game->xbefore = x - game->x_min_piece;
				game->ybefore = y - game->y_min_piece;
				find_the_ennemy(game);
